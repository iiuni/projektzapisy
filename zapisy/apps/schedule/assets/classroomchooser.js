...
let reservationData = JSON.parse(
  $("#reservation-data")[0].innerHTML
);
reservationData.canManageEvents = true;
...
var Classroom = function (ctx, item) {
    var config = {
        ...
        calculate: function () {
          ...
          var getPixel = function (hour) {
            /*
                                 ...
                                 */
            var cellSize =
              that.width / (that.evenHours.length - 1);
            var time = hour.split(":"),
              hours =
                ((parseInt(time[0]) - 8) * cellSize) / 2,
              minutes =
                Math.floor(
                  (parseInt(time[1]) / 60) * cellSize
                ) / 2;
      ...
      cellSize =
        config.width / (config.evenHours.length - 1),
      drawButton = function () {
      ...
      click = function (event, cordinates, callback) {
        if (!reservationData.canManageEvents && !is_free())
          return;
        if (
          cordinates.x >= config.x1 - 100 &&
          cordinates.x < config.x1 - 20 &&
          cordinates.y >= config.y1 + 1 &&
          cordinates.y < config.y1 + 32
        ) {
      ...
      is_free = function (force) {
        ...
        for (var key in config.terms) {
          if (config.terms.hasOwnProperty(key)) {
            ...
            if (
              !(
                lpixel >= pitem.end || rpixel <= pitem.begin
              )
            ) {
              return false;
            }
    ...
    return {
      ...
      shouldBeShow: function (l, r) {
        setLimit(
          getPixel(l) + config.x1,
          getPixel(r) + config.x1
        );
        return is_free(true);
      },
      setLimit: setLimit,
    ...
    var that = this,
      ...
      createClassroom = function (data) {
        for (var classroom in data) {
          if (data.hasOwnProperty(classroom)) {
            classrooms.push(
              new Classroom(that.ctx, data[classroom])
            );
          }
        }
      },
      ...
      draw = function () {
        ...
        for (var classroom in classrooms) {
          if (
            classrooms.hasOwnProperty(classroom) &&
            (shouldBeShow(classrooms[classroom]) ||
              $("#filteroccupated").prop("checked"))
          ) {
            toDraw[index] = classroom;
            index++;
          }
        }
        that.canvas.height = 65 * (index + 0.5);
        ...
        for (var c in toDraw) {
          if (toDraw.hasOwnProperty(c)) {
            if (index === 0) {
              if (lineFrom) {
                lPixel = classrooms[toDraw[c]].getPixel(
                  lineFrom
                );
              }
              if (lineTo) {
                rPixel = classrooms[toDraw[c]].getPixel(
                  lineTo
                );
              }
            }
