...
function createEmptyTerm(): Term {
    ...
    const day = timeStampFrom.date() === timeStampTo.date()
...
export default class ReservationEditor extends Vue {
    ...
    ensureValidActiveTerm(): void {
        ...
        const firstVisibleIndex = this.terms.findIndex((term: Term) => !term.deleted);
    ...
    setLocationTab(tab: LocationTab): void {
        ...
        if (tab === "outside" && this.activeTerm) {
            this.outsidePlaceInput = this.activeTerm.roomId === null ? this.activeTerm.place : "";
        }
    }
...
<template>
    <div>
        ...
        <input type="hidden" name="term_set-INITIAL_FORMS" :value="initialFormsCount" />
        ...
        <div v-for="(term, index) in terms" :key="index" class="d-none">
            ...
            <input type="hidden" :name="fieldName(index, 'start')" :value="term.start" />
            <input type="hidden" :name="fieldName(index, 'end')" :value="term.end" />
            <input type="hidden" :name="fieldName(index, 'place')" :value="term.place" />
            <input type="hidden" :name="fieldName(index, 'room')" :value="serializeValue(term.roomId)" />
            <input type="hidden" :name="fieldName(index, 'id')" :value="serializeValue(term.id)" />
            <input type="hidden" :name="fieldName(index, 'DELETE')" :value="term.deleted ? 'on' : ''" />
        </div>
        ...
        <div class="mb-3">
            ...
            >
                ...
                <div v-if="getTermErrors(entry.term).length > 0" class="px-1 pb-2">
                    <div class="text-danger fw-medium lh-sm" role="alert" aria-live="polite">
                        <div
        ...
        <div>
            ...
            <div class="tab-content mt-3">
                <div v-show="activeLocationTab === 'inside'" class="tab-pane fade show active">
                    <ClassroomPicker
                        v-if="activeTerm"
                ...
                <div v-show="activeLocationTab === 'outside'" class="tab-pane fade show active">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10 input-group">
                                ...
                                <div class="input-group-append">
                                    <button type="button" class="btn btn-info" @click="applyOutsidePlace">
                                        Wybierz
                                    </button>
                                </div>
                            </div>
