...
class OpeningTimesTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        GroupOpeningTimes.objects.filter(student=cls.tola, group=self.knitting_lecture_group).update(time=tola_knitting_opening + 2 * cls.diff)
    ...
    def test_selected_populated_times(self):
        ...
        lolek_knitting_opening = GroupOpeningTimes.objects.get(
            student=self.lolek, group=self.knitting_lecture_group).time
        assert bolek_knitting_opening - lolek_knitting_opening == 2 * cls.diff
