...
class GuaranteedSpotsEnrollmentTest(TestCase):
    ...
    def test_isim_not_involved(self):
        ...
        with freeze_time(self.opening_time + timedelta(seconds=5)):
            engine.enqueue_student(self.bolek, self.group)
        with freeze_time(self.opening_time + timedelta(seconds=10)):
            engine.enqueue_student(self.lolek, self.group)
        with freeze_time(self.opening_time + timedelta(seconds=15)):
            engine.enqueue_student(self.reksio, self.group)

        self.assertTrue(engine.is_enrolled(self.bolek, self.group))
        self.assertTrue(engine.is_enrolled(self.lolek, self.group))
        self.assertFalse(engine.is_enrolled(self.reksio, self.group))
        self.assertTrue(engine.is_recorded(self.reksio, self.group))

        with freeze_time(self.opening_time + timedelta(minutes=5)):
            engine.remove_from_group(self.lolek, self.group)

        self.assertTrue(engine.is_enrolled(self.bolek, self.group))
        self.assertTrue(engine.is_enrolled(self.reksio, self.group))
        self.assertFalse(engine.is_recorded(self.lolek, self.group))
    ...
    def test_one_isim_student(self):
        ...
        with freeze_time(self.opening_time + timedelta(minutes=1)):
            engine.enqueue_student(self.tola, self.group)
        ...
        self.assertFalse(engine.is_enrolled(self.reksio, self.group))
        self.assertTrue(engine.is_recorded(self.reksio, self.group))
        self.assertTrue(engine.is_enrolled(self.tola, self.group))
        ...
        self.assertTrue(engine.is_recorded(self.lolek, self.group))
        self.assertFalse(engine.is_enrolled(self.lolek, self.group))

        with freeze_time(self.opening_time + timedelta(minutes=15)):
            engine.remove_from_group(self.tola, self.group)

        # Now that Tola has left, the ISIM spot is free, but Lolek is still in
        # the queue.
        self.assertFalse(engine.is_recorded(self.tola, self.group))
        self.assertTrue(engine.is_recorded(self.lolek, self.group))
        self.assertFalse(engine.is_enrolled(self.lolek, self.group))
    ...
    def test_two_isim_students(self):
        ...
        with freeze_time(self.opening_time + timedelta(seconds=25)):
            engine.enqueue_student(self.uszatek, self.group)
        # Now both Reksio and Uszatek are waiting.
        self.assertFalse(engine.is_enrolled(self.reksio, self.group))
        self.assertFalse(engine.is_enrolled(self.uszatek, self.group))
        ...
        # in line.
        self.assertFalse(engine.is_enrolled(self.reksio, self.group))
        self.assertTrue(engine.is_enrolled(self.uszatek, self.group))
