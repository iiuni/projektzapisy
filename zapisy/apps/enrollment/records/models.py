...
class Record(models.Model):
    ...
    RECORD_STATUS = [(STATUS_REMOVED, u'usunięty'), (STATUS_ENROLLED,
                                                     u'zapisany'), (STATUS_PINNED, u'przypięty')]
...
#        self.save()
    ...
    def get_student_records_for_course(student, course):
        return Record.objects.filter(
            ...
            group__course=course).select_related('group')

    @staticmethod
    def get_student_records_for_group_type(student, group):
        return Record.get_student_records_for_course(
            student, group.course).filter(
            group__type=group.type)
    ...
    def get_groups_with_records_for_course(slug, user_id, group_type):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    'Record.get_groups_with_records_for_course(slug = %s, user_id = %d, group_type = %s) throws Student.DoesNotExist exception.' %
                    (unicode(slug), int(user_id), unicode(group_type)))
                student_groups = {}
            for g in groups:
                ...
                g.is_in_diff = Record.is_student_in_course_group_type(
                    user=user, slug=slug, group_type=group_type)
    ...
    def get_groups_for_student(user):
        try:
            ...
            logger.warning(
                'Record.get_groups_for_student(user_id = %d) throws Student.DoesNotExist exception.' % int(
                    user.id))
    ...
    def is_student_in_course_group_type(user, slug, group_type):
        try:
            course = Course.objects.get(slug=slug)
            user_course_group_type = [g.id for g in Record.get_groups_for_student(
                user) if g.course == course and g.type == group_type]
            if user_course_group_type:
                return user_course_group_type[0]
            return False
        except Student.DoesNotExist:
            logger.warning(
                'Record.is_student_in_course_group_type(slug = %s, user_id = %d, group_type = %s) throws Student.DoesNotExist exception.' %
                (unicode(slug), int(
                    user.id), unicode(group_type)))
            raise NonStudentException()
        except Course.DoesNotExist:
            logger.warning(
                'Record.is_student_in_course_group_type(slug = %s, user_id = %d, group_type = %s) throws Course.DoesNotExist exception.' %
                (unicode(slug), int(
                    user.id), unicode(group_type)))
            raise NonCourseException()
    ...
    def pin_student_to_group(user_id, group_id):
        ...
        try:
            ...
            record, is_created = Record.objects.get_or_create(
                group=group, student=student, status=Record.STATUS_PINNED)
            if not is_created:
                logger.warning(
                    'Record.pin_student_to_group(user_id = %d, group_id = %d) raised AlreadyPinnedException exception.' %
                    (int(user_id), int(group_id)))
                raise AlreadyPinnedException()
            logger.info(
                'User %s <id: %s> is now pinned to group: "%s" <id: %s>' %
                (user.username, user.id, group, group.id))
            return record
        except Student.DoesNotExist:
            logger.warning(
                'Record.pin_student_to_group(user_id = %d, group_id = %d) throws Student.DoesNotExists exception.' %
                (int(user_id), int(group_id)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Record.pin_student_to_group(user_id = %d, group_id = %d) throws Group.DoesNotExists exception.' %
                (int(user_id), int(group_id)))
            raise NonGroupException()
    ...
    def unpin_student_from_group(user_id, group_id):
        ...
        try:
            ...
            record.delete()
            logger.info(
                'User %s <id: %s> is no longer pinned to group: "%s" <id: %s>' %
                (user.username, user.id, group, group.id))
            return record
        except Record.DoesNotExist:
            logger.warning(
                'Record.unpin_student_from_group(user_id = %d, group_id = %d) throws Record.DoesNotExist exception' %
                (int(user_id), int(group_id)))
            raise AlreadyNotPinnedException()
        except Student.DoesNotExist:
            logger.warning(
                'Record.unpin_student_from_group(user_id = %d, group_id = %d) throws Student.DoesNotExist exception' %
                (int(user_id), int(group_id)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Record.unpin_student_from_group(user_id = %d, group_id = %d) throws Group.DoesNotExist exception' %
                (int(user_id), int(group_id)))
            raise NonGroupException()
            ...
            for l in lectures:
                ...
                if (l not in groups) and (l.get_count_of_enrolled(dont_use_cache=False) < l.limit):
                    ...
                    try:
                        ...
                        pass
                    logger.info(
                        'User %s <id: %s> is automaticaly added to lecture <id: %s> of Course: [%s] <id: %s>' %
                        (user.username, user.id, l.id, course.name, course.id))
                    backup_logger.info(
                        '[02] user <%s> is automaticaly added to lecture group <%s>' %
                        (user.id, l.id))
            return new_records
        except Student.DoesNotExist:
            logger.warning(
                'Record.add_student_to_lecture_group()  throws Student.DoesNotExist exception (parameters: user_id = %d, course_id = %d)' %
                (int(
                    user.id), int(
                    course.id)))
            raise NonStudentException()
    ...
    def __unicode__(self):
        return u"%s (%s - %s)" % (self.group.course,
...
class Queue(models.Model):
    ...
    priority = models.PositiveSmallIntegerField(
        default=1, validators=[queue_priority], verbose_name='priorytet')
...
#        return queues
    ...
    def get_priority(user_id, group_id):
        ...
        try:
            ...
            logger.warning(
                'Queue.get_priority() throws Student.DoesNotExist(parameters : user_id = %d, group_id = %d)' %
                (int(user_id), int(group_id)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Queue.get_priority() throws Group.DoesNotExist(parameters : user_id = %d, group_id = %d)' %
                (int(user_id), int(group_id)))
            raise NonGroupException()
    ...
    def get_students_in_queue(group_id):
        ...
        try:
            qq = Queue.objects.filter(
                deleted=False, group_id=group_id).select_related(
                'student', 'student__user').order_by('time')
            p = []
            for q in qq:
                p.append(q.student)
            return p
            ...
            logger.warning(
                'Queue.get_students_in_queue() throws Group.DoesNotExist(parameters : group_id = %d)' %
                int(group_id))
            raise NonGroupException()
    ...
    def change_student_priority(user_id, group_id, new_priority):
        ...
        try:
            ...
            record.save()
            logger.info('User %s <id: %s> changed queue priority of group "%s" <id: %s> to %s' % (
                student.user.username, student.user.id, record.group, record.group.id, new_priority))
            return record
        except Queue.DoesNotExist:
            logger.warning(
                'Queue.change_student_priority() throws Queue.DoesNotExist exception (parameters: user_id = %d, group_id = %d, new_priority = %d)' %
                (int(user_id), int(group_id), int(new_priority)))
            raise AlreadyNotAssignedException()
        except Student.DoesNotExist:
            logger.warning(
                'Queue.add_student_to_queue()  throws Student.DoesNotExist exception (parameters: user_id = %d, group_id = %d, new_priority = %d)' %
                (int(user_id), int(group_id), int(new_priority)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Queue.add_student_to_queue()  throws Group.DoesNotExist exception (parameters: user_id = %d, group_id = %d, new_priority = %d)' %
                (int(user_id), int(group_id), int(new_priority)))
            raise NonGroupException()
    ...
    def remove_student_from_queue(user_id, group_id):
        ...
        try:
            ...
            record.delete()
            logger.info(
                'User %s <id: %s> is now removed from queue of group "%s" <id: %s>' %
                (student.user.username, student.user.id, group, group.id))
            return record
        except Queue.DoesNotExist:
            logger.warning(
                'Queue.remove_student_from_queue() throws Queue.DoesNotExist exception (parameters: user_id = %d, group_id = %d)' %
                (int(user_id), int(group_id)))
            raise AlreadyNotAssignedException()
        except Student.DoesNotExist:
            logger.warning(
                'Queue.remove_student_from_group() throws Student.DoesNotExist exception (parameters: user_id = %d, group_id = %d)' %
                (int(user_id), int(group_id)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Queue.remove_student_from_group() throws Group.DoesNotExist exception (parameters: user_id = %d, group_id = %d)' %
                (int(user_id), int(group_id)))
            raise NonGroupException()
    ...
    def get_groups_for_student(user):
        ...
        try:
            ...
            logger.warning(
                'Queue.get_groups_for_student(user_id = %d) throws Student.DoesNotExist exception.' % int(
                    user.id))
            raise NonStudentException()
    ...
    def remove_student_low_priority_records(user_id, group_id, priority):
        ...
        try:
            ...
            queued_group = [g for g in Queue.get_groups_for_student(
                user) if g.course == course and g.type == group.type]
            ...
            for q_g in queued_group:
                ...
                if (record.priority <= priority):
                    logger.info(
                        'User %s <id: %s> is now removed from queue of group "%s" <id: %s> because of low priority (%s)' %
                        (user.username, user.id, q_g, q_g.id, priority))
                    Queue.remove_student_from_queue(user_id, q_g.id)
        except Student.DoesNotExist:
            logger.warning(
                'Queue.remove_student_low_priority_records throws Student.DoesNotExist exception (parameters user_id = %d, group_id = %d, priority = %d)' %
                (int(user_id), int(group_id), int(priority)))
            raise NonStudentException()
        except Group.DoesNotExist:
            logger.warning(
                'Queue.remove_student_low_priority_records throws Group.DoesNotExist exception (parameters user_id = %d, group_id = %d, priority = %d)' %
                (int(user_id), int(group_id), int(priority)))
            raise NonGroupException()
