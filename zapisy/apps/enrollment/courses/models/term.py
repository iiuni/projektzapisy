...
class Term(models.Model):
    ...
    def get_groups_terms(groups_ids):
        ...
        return map(
            lambda x: {
                'group_id': x.group_id,
                'term_as_string': "%s %s-%s (s.%s)" % (x.get_dayOfWeek_display_short(),
                                                       x.start_time.strftime("%H:%M"),
                                                       x.end_time.strftime("%H:%M"),
                                                       x.classrooms_as_string)},
            Term.objects.filter(
                group__in=groups_ids).extra(
                select={
                    'classrooms_as_string': """
                                    SELECT array_to_string(array(SELECT courses_classroom.number FROM courses_term_classrooms
                                    JOIN courses_classroom
                                    ON (courses_classroom.id = courses_term_classrooms.classroom_id)
                                    WHERE courses_term.id=courses_term_classrooms.term_id),',')"""}))
...
def log_edit_term(sender, instance, **kwargs):
    ...
    try:
        ...
        message = '[09] term for group <%s> has been updated from <%s> to <%s>' % (
            term.group.id, old_term_format.encode('utf-8'), term_format.encode('utf-8'))
...
def log_add_term(sender, instance, created, **kwargs):
    ...
    if created:
        message = '[08] term <%s> for group <%s> has been created' % (
            term_format.encode('utf-8'), term.group.id)
        backup_logger.info(message)
...
def log_delete_term(sender, instance, **kwargs):
    ...
    backup_logger.info('[10] term <%s> for group <%s> has been deleted' %
                       (term_format.encode('utf-8'), term.group.id))
