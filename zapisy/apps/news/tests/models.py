...
class NewsManagerTest(TestCase):
    def test_new(self):
        ...
        def f(o): return o.id
        nss = list(map(f, News.objects.new('-')))
        ncs = list(map(f, sorted(ns,
                key=(lambda o:o.date), reverse=True)[0:max(new,3)]))
        self.assertEqual(nss, ncs)
        for n in ns:
            n.delete()

    def test_count_new(self):
        self.assertEqual(0, News.objects.count_new('-'))
        (new, old, ns) = generate_random_news('-')
        self.assertEqual(new, News.objects.count_new('-'))
        for n in ns:
            n.delete()
    ...
    def test_count_new_categories(self):
        ...
        self.assertEqual(new1, News.objects.count_new('1'))
        self.assertEqual(new2, News.objects.count_new('2'))
        for n in ns1 + ns2:
            n.delete()
