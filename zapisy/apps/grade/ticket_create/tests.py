...
class UtilsTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        user2 = UserFactory()
        cls.employee = EmployeeFactory(user=user1)
        cls.student = StudentFactory(user=user2)
    ...
    def test_generate_keys_for_polls_generates_keys_for_new_polls_only(self):
        ...
        new_poll = Poll(author=Employee.objects.get(pk=self.employee.pk),
                        title="test",
                        description="brak",
    ...
    def test_group_polls_by_course_makes_valid_groups(self):
        generate_keys_for_polls()
        polls = Poll.get_all_polls_for_student(Student.objects.get(pk=self.student.pk))
