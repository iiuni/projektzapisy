        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                signed = map(
                    lambda g_t: (
                        g_t[0], long(
                            g_t[1]), secure_signer_without_save(
                            request.user, g_t[0], long(
                                g_t[1]))), tickets)
                unblinds = map(lambda g_t_st:
                               (unicode(g_t_st[1]), unblind(g_t_st[0], g_t_st[2])),
                               signed)
                        ...
                        for poll in polls:
                            ...
                            if not poll.group:
                                ...
                                st += u'Przedmiot: ' + polls[0].group.course.name + u'%%%    [' + poll.title + u'] ' + \
                                    poll.group.get_type_display() + u': ' + poll.group.get_teacher_full_name() + u'***'
