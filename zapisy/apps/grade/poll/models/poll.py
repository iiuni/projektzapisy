...
class Poll(models.Model):
    ...
    def __unicode__(self):
        ...
        if self.group:
            res += u', ' + unicode(self.group.get_type_display()) + u" - " + \
                unicode(self.group.get_teacher_full_name())
        if self.studies_type:
            res += u', typ studiów: ' + unicode(self.studies_type)
        return res
    ...
    def get_all_polls_for_student(student):
        ...
        return filter(
            lambda x: not x.group or x.group.id in groups,
