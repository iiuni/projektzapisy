...
def prepare_data(request, slug):
    ...
    dict = {}
    if polls:
        polls_id = reduce(lambda x, y: x + y, map(lambda x_s_l:
                                                  map(lambda id_t_st:
                                                      id_t_st[0],
                                                      x_s_l[1]), polls))
        ...
        data['polls'] = map(lambda x_s_l7:
                            ((x_s_l7[0][0], x_s_l7[0][1]),
                                slug == x_s_l7[0][1],
                                map(lambda id_t_st2:
                                    (id_t_st2[0], id_t_st2[1], id_t_st2[2], dict[id_t_st2[0]].to_url_title(True)),
                                    x_s_l7[1])),
                            polls)
    else:
        data['polls'] = []
    ...
    if finished:
        finished_id = reduce(lambda x, y: x + y, map(lambda x_s_l4:
                                                     map(lambda id_t_st1:
                                                         id_t_st1[0],
                                                         x_s_l4[1]), finished))
        for poll in Poll.objects.filter(
        ...
        data['finished'] = map(lambda x_s_l8:
                               ((x_s_l8[0][0], x_s_l8[0][1]),
                                slug == x_s_l8[0][1],
                                map(lambda id_t_st3:
                                    (id_t_st3[0], id_t_st3[1], id_t_st3[2], dict[id_t_st3[0]].to_url_title(True)),
                                    x_s_l8[1])),
                               finished)
    else:
        data['finished'] = []
    ...
    data['all_polls'] = reduce(lambda x, y: x + y, map(lambda p_l: len(p_l[1]),
...
def get_ticket_and_signed_ticket_from_session(session, slug, poll_id):
    ...
    polls = flatten(map(lambda n_s_lt: n_s_lt[1], filter(
        lambda name_s_lt: name_s_lt[0][1] == slug, polls)))
    finished = flatten(map(lambda n_s_lt9: n_s_lt9[1], filter(
        lambda name_s_lt5: name_s_lt5[0][1] == slug, finished)))

    data = polls + finished
    data = map(
        lambda id_t_st10: (
            id_t_st10[1], id_t_st10[2]), filter(
            lambda id_t_st6: id_t_st6[0] == int(poll_id), data))
