...
def polls_for_user(request, slug):
    if 'polls' not in request.session.keys():
        return HttpResponseRedirect(reverse('grade-poll-tickets-enter'))
    ...
    if ticket and signed_ticket and check_signature(ticket, signed_ticket, public_key):
        ...
        if request.method == "POST" and not st.finished:
            ...
            if errors:
                ...
                messages.error(
                    request,
                    u"Nie udało się zapisać ankiety: " +
                    poll.title +
                    u"; błąd formularza")
            else:
                data['form_errors'] = {}
                ...
                if 'finish' in request.POST:
                    ...
                    slug_polls = filter(lambda x_s_ls: slug == x_s_ls[0][1], polls)
                    slug_finit = filter(lambda x_s_ls1: slug == x_s_ls1[0][1], finit)
    ...
    if poll_id:
        ...
        if poll.is_user_entitled_to_view_result(request.user):
            ...
            if semester.is_grade_active:
                ...
                data['completness'] = SafeUnicode(
                    u"Liczba studentów, którzy wypełnili ankietę: %d" % (sts_fin))
        ...
        if not st_id:
            ...
            for i, st in enumerate(sts):
                if unicode(st.id) == unicode(st_id):
                    ...
                    break
            if err:
                messages.error(request, u"Nie istnieje taka odpowiedź na ankietę")
...
def save_csv(request, mode, poll_id):
    ...
    sections = []
    for section, section_answers in poll_answers:
        sections.append(map(lambda q: u'[' + unicode(section.title) +
                            u']' + unicode(q.content), section.all_questions()))
