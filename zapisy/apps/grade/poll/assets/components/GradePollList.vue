<template>
  <div class="accordion" id="course-sections">
    ...
    <div v-for="(entries, group_name, index) in currentList" :key="group_name" class="card">
      <div
        class="card-header"
      ...
      >
        <div class="d-flex w-100 justify-content-between">
          ...
          <span class="text-right text-nowrap">{{ submissionsCount[group_name] || 0 }}</span>
        </div>
      </div>
      <div
      ...
      >
        <a
          ...
          :key="entry.id"
          :href="baseHtml + '/grade/poll/results/semester/' + selectedSemester.id + '/poll/' + entry.id"
