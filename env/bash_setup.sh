...
echo "alias initial-migration='python manage.py makemigrations && python manage.py migrate --fake-initial'" >> .bash_aliases
echo "alias development-setup='mkdir /vagrant/zapisy/logs && get-settings && sudo su - postgres -c \"psql -f /vagrant/db_dump.sql fereol\" && cd /vagrant/zapisy && initial-migration'" >> .bash_aliases
